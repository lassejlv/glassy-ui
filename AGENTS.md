...
- This is one `grafik-ui` package, not a workspace. `src/lib.rs` is the library surface; `src/main.rs` is the `grafik-gallery` demo binary and default run target.
...
- A consuming app must install `Assets`, then call `init_motion`, `init_theme`, `grafik_ui::init` (input keybindings), and `load_fonts`; `src/main.rs` is the executable reference.

## Commands

- Run the gallery: `cargo run` (equivalent to `cargo run --bin grafik-gallery`).
