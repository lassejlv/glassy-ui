...
use super::slot::StyledSlot;
use super::style::{apply_style, MotionStyle, ResolvedStyle};
use super::transition::{Ease, Transition};
use super::values::MotionValue;
...
impl Motion {
    ...
    /// Exit style used by [`crate::AnimatePresence`].
    ...
    /// Quiet feedback for marks that appear after selecting a control.
    pub fn selection_in(self) -> Self {
        self.initial(MotionStyle::new().opacity(0.))
            .animate(MotionStyle::new().opacity(1.))
            .transition(Transition::tween(Duration::from_millis(140)).ease(Ease::EaseOut))
    }

    /// Material surface arrival without bounce or geometry movement.
    pub fn surface_in(self) -> Self {
        self.initial(MotionStyle::new().opacity(0.))
            .animate(MotionStyle::new().opacity(1.))
